...
void EngineBufferScaleRubberBand::prepareChannelBuffers() {
    // Must only be called from non-real-time paths (constructor,
    // setSignal chain via onSignalChanged, or useEngineFiner).
...
void EngineBufferScaleRubberBand::onSignalChanged() {
    ...
    // path, but may also be called from RT contexts. Buffer allocation is
    // done in prepareChannelBuffers() which must be called before any RT
    // code runs (from setSignal/setup, constructor, or useEngineFiner).
